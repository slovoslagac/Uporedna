	...
	<table>
		...
		<?php foreach ($Data1 as $mat) { 
			...
			<tr class="naslov">
				...
				<?php for($i=0; $i<=$numBookmakers; $i++) {?>
				<td class="rightb" colspan="4"><?php echo $mat['klad_1'.$i]?></td>
				<?php }?>
				<?php for($i=0; $i<=$numBookmakers; $i++) {?>
		...
		<?php } $code=$mat['code']; $st= new DateTime($mat['start_time']); $start_date= $st -> format('d.m') ;$start_time = $st -> format('H:i'); 
			...
			<tr class="row<?php echo($j++ & 1 )?>">
				...
				<td class="<?php echo ($fav == 3 && !empty($ki2) && $maxOdds[$code] == (double) $ki2) ? 'biggestr' : ''; ?>"><?php echo $ki2?></td>
				<td class="nd"><?php echo ($ki1!=0 && $ki2!=0 && $kix!=0) ? LevelOdds($ki1, $kix, $ki2) : ""?></td>
				<?php }?>
